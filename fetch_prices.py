...
def fetch_ticker(sym):
    ...
    Some symbols fail on .info due to yfinance cache — 
    fall back to history-only which always works.
    """
    yf_sym = to_yf(sym)
    info, hist = {}, None
    ...
    for attempt in range(3):
        try:
            ...
            # Try info separately — sometimes 404s on info but history works
            try:
                info = t.info or {}
            except Exception:
                info = {}
