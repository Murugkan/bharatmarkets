...
def load_symbols():
    """Load symbols from symbols.json — single source of truth."""
    try:
...
def main():
    ...
    # Single symbol mode: merge into existing prices.json
    if single:
        try:
            existing = json.loads(Path(PRICES_FILE).read_text()).get("quotes",{})
    ...
    # Update symbols.json metadata from live data (only for full runs)
    if not single:
