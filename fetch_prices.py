...
SPECIAL_MAP = {
    ...
    "TRUALTBIO":     "TRUALT.NS",
    "ZENTECH":       "ZENTEC.NS",
    "KWALYPH":       "KPL.BO",          # CDSL truncated → Kwality Pharmaceuticals
    "KWALITYPHARM":  "KPL.BO",
    "CAPITALNUMBE":  "CNINFOTECH.BO",   # CDSL truncated → CapitalNumbers Infotech
    "CAPITALNUMB":   "CNINFOTECH.BO",
    "AMARAJABAT":    "ARE&M.NS",
        ...
        try:
            ...
            for qt in quotes:
                ...
                if (sym_yf.endswith(".NS") or sym_yf.endswith(".BO")) and qt.get("quoteType","") in ("EQUITY",""):
                    ...
                    # If BSE-only, add to SPECIAL_MAP so to_yf() returns correct exchange
                    if sym_yf.endswith(".BO"):
                        SPECIAL_MAP[sym] = sym_yf
...
def load_symbols():
    """Load symbols from symbols.json — resolve names via Yahoo when not yet confirmed."""
    try:
        data = json.loads(Path(SYMBOLS_FILE).read_text())
        resolved_any = False
        for entry in data:
            # Re-resolve if not confirmed yet (resolved:false or missing)
            if entry.get("resolved") is True:
                continue
            name = entry.get("name","")
            isin = entry.get("isin","")
            if not name:
                entry["resolved"] = True  # nothing to resolve
                continue
            ...
            if sym:
                ...
                # Keep existing sym as fallback, mark resolved to avoid retry loop
                entry["resolved"] = True
                print(f"  ⚠ Could not resolve '{name}' — keeping sym={entry.get('sym','?')}")
        if resolved_any:
...
def main():
    ...
    # Only remove stale data when explicitly requested (delete action)
    if os.environ.get("CLEAN_STALE", "").lower() in ("true", "1", "yes"):
        active_syms = set(symbols)
        removed = [s for s in list(quotes) if s not in active_syms]
        for s in removed:
            del quotes[s]
            chart_file = CHARTS_DIR / f"{s}.json"
            if chart_file.exists(): chart_file.unlink()
        if removed: print(f"  🗑 cleaned: {', '.join(removed)}")
