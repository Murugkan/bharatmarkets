        ...
        try:
            ...
            for attr in ['quarterly_income_stmt', 'quarterly_financials']:
                try:
                    ...
                    if df is not None and not df.empty:
                        qf = df
                        print(f"  📊 {sym} {attr} rows: {list(df.index)}")
                        break
                ...
                for row_label in qf.index:
                    ...
                    for col in qf.columns:
                        ...
                        try:
                            ...
                            continue
                        if   'total revenue'     in rl: q_data[k]['rev']  = round(v/1e7, 2)
                        elif 'operating revenue' in rl: q_data[k].setdefault('rev', round(v/1e7, 2))
                        elif 'net income'        in rl: q_data[k]['net']  = round(v/1e7, 2)
                        elif 'basic eps'         in rl: q_data[k]['eps']  = round(v, 2)
                        elif 'diluted eps'       in rl: q_data[k].setdefault('eps', round(v, 2))
                        elif 'ebitda'            in rl: q_data[k]['ebitda']= round(v/1e7, 2)
                        elif 'operating income'  in rl: q_data[k]['ebit'] = round(v/1e7, 2)
                        elif 'gross profit'      in rl: q_data[k]['gross']= round(v/1e7, 2)
                        else: pass  # unmatched row — see 📊 print above
            else:
                ...
                for row_label in qc.index:
                    ...
                    for col in qc.columns:
                        ...
                        try:
                            ...
                            continue
                        if   'operating cash flow' in rl or ('operating' in rl and 'cash' in rl):
            ...
            for attr in ['quarterly_balance_sheet', 'quarterly_balancesheet']:
                try:
                    ...
                    if df is not None and not df.empty:
                        qb = df
                        print(f"  📊 {sym} {attr} rows: {list(df.index[:6])}")
                        break
                except Exception as ex:
                    print(f"  ⚠ {sym} {attr}: {ex}")
...
def main():
    ...
    # TEST_MODE: run just first 3 stocks to see debug output quickly
    if os.environ.get('TEST_MODE'):
        active_syms = active_syms[:3]
        print(f"🧪 TEST_MODE: running {len(active_syms)} stocks only")
