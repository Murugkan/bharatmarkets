    ...
    try:
        ...
        if sh:
            ...
            if tbl:
                ...
                # Print ALL shareholding rows so we can see exact structure
                print(f"  📋 {sym} shareholding rows:")
                for dbg in all_rows[:8]:
                    print(f"     {[c.get_text(strip=True) for c in dbg.find_all(['td','th'])]}")
                for row in all_rows:
