...
def resolve_yf_sym(nse_sym):
    ...
    if nse_sym in YF_ALIAS_CACHE:
        yf = YF_ALIAS_CACHE[nse_sym]
        return yf if "." in yf or yf.startswith("^") else yf + ".NS"
    if nse_sym in NSE_TO_YAHOO:
        yf = NSE_TO_YAHOO[nse_sym]
        result = yf if "." in yf or yf.startswith("^") else yf + ".NS"
        ...
        if hist_short is None or hist_short.empty:
            try:
                ...
                if hist_bo is not None and not hist_bo.empty:
                    print(f"  ⚠ {nse_sym}: using BSE ticker {bo_sym}")
                    yf_sym = bo_sym
                    t = t_bo
                    hist_short = hist_bo
                    YF_ALIAS_CACHE[nse_sym] = bo_sym
                else:
                    print(f"  ✗ {nse_sym}: not found on NSE or BSE — skipping")
                    return result
            except Exception:
                print(f"  ✗ {nse_sym}: not found on Yahoo Finance — skipping")
                return result
