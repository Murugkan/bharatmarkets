...
def fetch_yfinance(sym):
    ...
    try:
        ...
        # Fetch history first — more reliable than .info for 404 symbols
        hist = None
        for attempt in range(3):
            try:
                ...
                print(f"  attempt {attempt+1} hist fail: {he}")
                time.sleep(1.5)
        ...
        if not ltp and hist is not None and not hist.empty:
            ...
            if len(closes) >= 1:
                ...
                print(f"  ⚠ using history price: ₹{ltp}")
