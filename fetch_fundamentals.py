...
def load_symbols():
    """Read symbols from symbols.json — resolve names to symbols if needed."""
    global CDSL_NAMES
    syms = []
    seen = set()
    try:
        ...
        for entry in data:
            ...
            name = entry.get("name","")
            isin = entry.get("isin","")
            # Resolve symbol from name if missing
            if not sym or len(sym) < 2:
                sym = search_yahoo_symbol(name, isin) or ""
                if sym:
                    entry["sym"] = sym
                    resolved_any = True
...
def main():
    ...
    # Remove stale symbols no longer in symbols.json
