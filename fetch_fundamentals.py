...
NSE_TO_YAHOO = {
    ...
    "TRUALTBIO":     "TRUALT",
    "ZENTECH":       "ZENTEC",
    "KWALYPH":       "KPL",             # CDSL truncated → Kwality Pharmaceuticals
    "KWALITYPHARM":  "KPL",
    "CAPITALNUMBE":  "CNINFOTECH",      # CDSL truncated → CapitalNumbers Infotech
...
def load_symbols():
    """Read symbols from symbols.json — resolve names via Yahoo when not yet confirmed."""
    global CDSL_NAMES
    syms = []
    seen = set()
    try:
        ...
        for entry in data:
            name = entry.get("name","")
            isin = entry.get("isin","")
            # Re-resolve if not confirmed yet
            if entry.get("resolved") is not True:
                sym = search_yahoo_symbol(name, isin) or entry.get("sym","").strip().upper()
                if sym:
                    entry["sym"]      = sym
                    entry["resolved"] = True
                    resolved_any = True
