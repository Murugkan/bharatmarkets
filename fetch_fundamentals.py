...
def fetch_yfinance(sym):
    ...
    try:
        ...
        # Try short history first (1mo is faster than 5y, confirms if listed)
        hist_short = None
        try:
        ...
        # If empty — symbol is delisted or invalid, skip immediately
        if hist_short is None or hist_short.empty:
            print(f"  ✗ {sym}: delisted or not found on NSE — skipping")
            return result
