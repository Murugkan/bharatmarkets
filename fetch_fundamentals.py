...
def fetch_nse_shareholding(sym):
    ...
    try:
        ...
        # DEBUG: print raw shareHolding keys for first stock
        if not getattr(fetch_nse_shareholding, "_debug_done", False):
            top_keys = list(data.keys())[:10]
            sh_keys  = list(sh.keys())[:10] if sh else []
            print(f"  🔍 NSE {sym} top keys: {top_keys}")
            print(f"  🔍 NSE {sym} shareHolding keys: {sh_keys}")
