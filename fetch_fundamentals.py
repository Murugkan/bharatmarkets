...
def fetch_nse_shareholding(sym):
    ...
    try:
        ...
        # Use quote-equity endpoint — most reliable NSE API, widely used
        # Contains shareHolding section with promoter% and pledge%
        ...
        # DEBUG: drill into each top-level key to find pledge/promoter data
        if not getattr(fetch_nse_shareholding, "_debug_done", False):
            ...
            for section in ["info","metadata","securityInfo","sddDetails","industryInfo"]:
                v = data.get(section)
                if isinstance(v, dict):
                    print(f"  🔍   {section} keys: {list(v.keys())[:12]}")
                    # Look for anything pledge/promoter related
                    for k in v.keys():
                        if any(x in k.lower() for x in ["pledge","promot","hold","share"]):
