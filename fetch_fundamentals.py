...
def fetch_nse_shareholding(sym):
    """Fetch shareholding data from NSE API."""
    result = {}
    try:
        ...
        # NSE trade_info confirmed: contains only order book + delivery data, no shareholding
        # Silently return empty — pledge sourced from Screener instead
...
def main():
    ...
    # Load pledge.json (written by fetch_pledge.py — Playwright headless browser)
    pledge_data = {}
    if Path("pledge.json").exists():
        try:
            pledge_data = json.loads(Path("pledge.json").read_text())
            print(f"📌 pledge.json loaded: {len(pledge_data)} entries\n")
        except Exception:
            pass
    ...
    for i, sym in enumerate(syms):
        ...
        # pledge.json — authoritative source (Playwright-rendered, overrides Screener)
        if sym in pledge_data:
            stock["pledge_pct"] = pledge_data[sym]
