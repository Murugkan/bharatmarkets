...
def fetch_yfinance(sym):
    ...
    try:
        ...
        try:
            ...
            # quarterly_earnings → EPS + Revenue
            qe = t.quarterly_earnings
            if qe is not None and not qe.empty:
                for d, row in qe.iterrows():
                    k = qkey(d)
                    if k not in q_data: q_data[k] = {}
                    v = safe_float(row.get("Earnings"))
                    if v is not None: q_data[k]["eps"] = round(v, 2)
                    r = safe_float(row.get("Revenue"))
                    if r is not None: q_data[k]["rev"] = round(r/1e7, 2)  # Cr

            # quarterly_financials → net profit, revenue (more reliable)
            qf = t.quarterly_financials
            if qf is not None and not qf.empty:
                idx = [str(i).lower() for i in qf.index]
                for i, label in enumerate(idx):
                    orig = qf.index[i]
                    for d in qf.columns:
                        k = qkey(d)
                        if k not in q_data: q_data[k] = {}
                        v = safe_float(qf.loc[orig, d])
                        if v is None: continue
                        if "net income" in label or "net profit" in label:
                            q_data[k]["net"] = round(v/1e7, 2)
                        elif "total revenue" in label or "revenue" in label:
                            if "net" not in q_data.get(k,{}):  # don't overwrite eps-based
                                q_data[k]["rev"] = round(v/1e7, 2)
                        elif "ebit" in label and "da" not in label:
                            q_data[k]["ebit"] = round(v/1e7, 2)
                        elif "gross profit" in label:
                            q_data[k]["gross"] = round(v/1e7, 2)

            # quarterly_cashflow → CFO, FCF
            qc = t.quarterly_cashflow
            if qc is not None and not qc.empty:
                idx = [str(i).lower() for i in qc.index]
                for i, label in enumerate(idx):
                    orig = qc.index[i]
                    for d in qc.columns:
                        k = qkey(d)
                        if k not in q_data: q_data[k] = {}
                        v = safe_float(qc.loc[orig, d])
                        if v is None: continue
                        if "operating" in label:
                            q_data[k]["cfo"] = round(v/1e7, 2)
                        elif "free" in label or "capital expenditure" in label:
                            if "cfo" in q_data.get(k,{}):
            ...
            if qb is not None and not qb.empty:
                idx = [str(i).lower() for i in qb.index]
                for i, label in enumerate(idx):
                    orig = qb.index[i]
                    if "total debt" in label or "long term debt" in label:
                        for d in qb.columns:
                            k = qkey(d)
                            if k not in q_data: q_data[k] = {}
                            v = safe_float(qb.loc[orig, d])
                            if v is not None:
                                q_data[k]["debt"] = round(v/1e7, 2)
                        break

            # Compute OPM per quarter where possible
            for k, v in q_data.items():
                if v.get("ebit") and v.get("rev") and v["rev"] != 0:
                    v["opm"] = round(v["ebit"]/v["rev"]*100, 1)

            if q_data:
                quarters = sorted(q_data.items())[-12:]  # keep 12 quarters = 3 years
                result["quarterly"] = [{"d": k, **v} for k, v in quarters]
                print(f"  ✓ quarterly: {len(quarters)}Q, fields: {set(k for _,v in quarters for k in v)}")
