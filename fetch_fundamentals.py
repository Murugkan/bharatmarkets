...
def fetch_nse_shareholding(sym):
    ...
    try:
        ...
        url   = (f"https://www.nseindia.com/api/corporate-share-holdingpattern"
        ...
        # DEBUG: print raw keys for first run to verify field names
        if not getattr(fetch_nse_shareholding, "_debug_done", False):
            latest_dbg = data[0] if isinstance(data, list) else data
            print(f"  🔍 NSE {sym} raw keys: {list(latest_dbg.keys())[:15]}")
            fetch_nse_shareholding._debug_done = True

        latest = data[0] if isinstance(data, list) else data

        # Try all known field name variants for pledge
        PLEDGE_KEYS = [
            "promoterAndPromoterGroupPledgedSharesPercentage",
            "pledgedShares", "pledge", "pPledge",
            "promoterPledge", "pledgedPercentage",
            "percentPledged",
        ]
        ...
        if result:
            print(f"  ✓ NSE {sym}: Pledge:{result.get('pledge_pct','—')}%")
        else:
            print(f"  ⚠ NSE {sym}: pledge field not found in response")
    except Exception as e:
        print(f"  ⚠ NSE {sym}: {e}")
            ...
            if tbl:
                for row in tbl.find_all("tr"):
                    ...
                    if "promoter" in lbl and "pledge" not in lbl:
                        result["prom_pct"] = val
                        # If no pledge row appears later, pledge = 0 (Screener omits 0% rows)
                        result.setdefault("pledge_pct", 0)
                    elif "pledge" in lbl:
                        result["pledge_pct"] = val   # overrides the 0 set above
                    elif "public" in lbl:
...
def main():
    ...
    for i, sym in enumerate(syms):
        ...
        # 2. NSE shareholding API — for pledge_pct (Screener doesn't expose it in HTML)
        nse_data = fetch_nse_shareholding(sym)
        if nse_data:
            stock["pledge_pct"] = nse_data["pledge_pct"]  # authoritative source
            stats["nse"] += 1
