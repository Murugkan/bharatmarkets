...
# Covers company renames, mergers, special characters
NSE_TO_YAHOO = {
    "AMARAJAB":    "ARE&M",
    "ASSALCOHOL":  "ASALCBR",
    "AZADINDIA":   "AZADINDIA",
...
def yahoo_search_sym(nse_sym):
    ...
    try:
        url = (f"https://query2.finance.yahoo.com/v1/finance/search"
               f"?q={nse_sym}&lang=en-IN&region=IN&quotesCount=5&newsCount=0")
        r = requests.get(url, headers=HEADERS, timeout=8)
        if r.status_code != 200:
            return None
        for q in r.json().get("quotes", []):
            sym_yf = q.get("symbol", "")
            if sym_yf.endswith(".NS"):
                print(f"  🔍 Yahoo search: {nse_sym} → {sym_yf}")
                YF_ALIAS_CACHE[nse_sym] = sym_yf
                NSE_TO_YAHOO[nse_sym] = sym_yf.replace(".NS", "")
                return sym_yf
    except Exception as e:
        print(f"  ⚠ Yahoo search {nse_sym}: {e}")
    return None
...
def fetch_yfinance(sym):
    ...
    try:
        ...
        # Still empty after search — truly delisted/not on Yahoo
        if hist_short is None or hist_short.empty:
            print(f"  ✗ {sym}: not found on Yahoo Finance — skipping")
            # Don't add to DELISTED — it may just be a symbol mismatch we haven't mapped yet
            return result
        ...
        try:
            h5 = t.history(period="max", interval="1mo", auto_adjust=True)
            if not h5.empty:
...
def main():
    ...
    for i, sym in enumerate(syms):
        ...
        # 2. NSE shareholding (overrides promoter/pledge with official data)
        nse_data = fetch_nse_shareholding(sym)
        if nse_data:
            for k in ["prom_pct","public_pct","fii_pct","dii_pct","pledge_pct"]:
                if nse_data.get(k) is not None:
                    stock[k] = nse_data[k]
            stats["nse"] += 1
        time.sleep(0.3)

        # 3. Screener.in — only if key fields still missing
        needs = any(stock.get(f) is None for f in [
            "prom_pct","pledge_pct","opm_pct","cfo","roce"
        ])
        ...
        print(f"  → Signal: {sig} | Pos:{pos} Neg:{neg} | {filled} fields total\n")
