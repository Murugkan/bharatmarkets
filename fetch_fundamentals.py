...
def fetch_yfinance(sym):
    ...
    try:
        ...
        # ── Quarterly history: EPS + CFO for chart panels ──────────────
        # Fetch last 8 quarters of earnings and cashflow
        # Stored as list of {d, eps, cfo} sorted oldest→newest
        try:
            q_earnings = t.quarterly_earnings   # index=quarter, cols=[Earnings,Revenue]
            q_cashflow = t.quarterly_cashflow   # index=metric, cols=quarters

            q_data = {}

            # Quarterly EPS
            if q_earnings is not None and not q_earnings.empty:
                for qdate, row in q_earnings.iterrows():
                    key = str(qdate)[:10]
                    eps_val = safe_float(row.get("Earnings"))
                    if eps_val is not None:
                        if key not in q_data:
                            q_data[key] = {}
                        q_data[key]["eps"] = round(eps_val, 2)

            # Quarterly CFO — row label varies: "Total Cash From Operating Activities" etc
            if q_cashflow is not None and not q_cashflow.empty:
                cfo_row = None
                for label in q_cashflow.index:
                    if "operating" in str(label).lower():
                        cfo_row = q_cashflow.loc[label]
                        break
                if cfo_row is not None:
                    for qdate, val in cfo_row.items():
                        key = str(qdate)[:10]
                        cfo_val = safe_float(val)
                        if cfo_val is not None:
                            if key not in q_data:
                                q_data[key] = {}
                            q_data[key]["cfo"] = round(cfo_val / 1e7, 2)  # → Crores

            if q_data:
                # Sort by date, keep last 8 quarters
                quarters = sorted(q_data.items())[-8:]
                result["quarterly"] = [{"d": k, **v} for k, v in quarters]

        except Exception:
            pass  # quarterly data optional — don't fail main fetch
