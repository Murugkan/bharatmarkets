...
def fetch_screener_gaps(sym):
    ...
    try:
        ...
        if sh:
            tbl = sh.find("table")
            if tbl:
                if not getattr(fetch_screener_gaps, "_sh_debug_done", False):
                    fetch_screener_gaps._sh_debug_done = True
                    print(f"  📋 Screener {sym} shareholding rows:")
                    for dbg_row in tbl.find_all("tr")[:10]:
