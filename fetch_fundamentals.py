...
def fetch_screener_gaps(sym):
    ...
    try:
        ...
        if sh:
            tbl = sh.find("table")
            if tbl:
                # DEBUG: print first 8 rows raw so we can see exact Screener structure
                all_rows = tbl.find_all("tr")
                if not getattr(fetch_screener_gaps, "_debug_done", False):
                    print(f"  🔍 DEBUG shareholding rows for {sym}:")
                    for dbg_row in all_rows[:8]:
                        dbg_cells = [c.get_text(strip=True) for c in dbg_row.find_all(["td","th"])]
                        print(f"     {dbg_cells}")
                    fetch_screener_gaps._debug_done = True
                for row in all_rows:
                    cells = [c.get_text(strip=True) for c in row.find_all(["td","th"])]
                    if len(cells) < 2:
                        continue
                    lbl = cells[0].strip().lower()
                    # Latest quarter is the last numeric cell (rightmost column)
                    # Skip header rows (cells that are all text like "Sep 2025")
                    numeric_cells = [c for c in cells[1:] if safe_float(c.replace("%","").replace(",","")) is not None]
                    if not numeric_cells:
                        continue
                    val = safe_float(numeric_cells[-1].replace("%","").replace(",",""))
                    if val is None:
                        continue
                    # Match row types — Screener uses "Promoters", "Pledged", "FIIs", "Public"
                    if ("promoter" in lbl) and ("pledge" not in lbl) and ("pledged" not in lbl):
                        result["prom_pct"]   = val
