...
def load_symbols():
    ...
    # Cross-check against fundamentals.json
    try:
        ...
        if skipped:
            print(f"Skipping {len(skipped)} without fundamentals: {', '.join(skipped)}")
        print(f"Active symbols ({len(valid)}): {', '.join(valid)}")
        return valid
    except:
        print("fundamentals.json not found -- using all symbols")
        return syms
