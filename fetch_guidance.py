...
2. Sends headlines + snippets to Claude API
3. Extracts structured forward guidance
4. Stores in guidance.json
...
def load_symbols():
    """Load active symbols from prices.json or portfolio_symbols.txt.
    Excludes index symbols. Cross-checks against fundamentals.json.
    """
    single = os.environ.get("SINGLE_SYMBOL","").strip().upper()
    if single:
        return [single.split("|")[0].strip()]

    INDEX_SYMS = {"NIFTY","BANKNIFTY","NIFTYMIDCAP100","CNXIT","NIFTYPSE",
                  "NIFTYSMALLCAP100","SENSEX","NIFTY50"}
    syms, seen = [], set()

    def add(s):
        s = s.split("|")[0].strip().upper()
        if s and s not in seen and s not in INDEX_SYMS:
            syms.append(s); seen.add(s)

    # 1. portfolio_symbols.txt
    ...
    if p.exists():
        for line in p.read_text().splitlines():
            ...
            if line and not line.startswith("#"):
                add(line)
        print(f"portfolio_symbols.txt: {len(syms)} symbols")

    # 2. prices.json -- most reliable source of what is actually tracked
    if not syms:
        try:
            prices = json.loads(Path("prices.json").read_text())
            quotes = prices.get("quotes", prices)
            for s in quotes.keys():
                add(s)
            print(f"prices.json: {len(syms)} symbols")
        except Exception as e:
            print(f"Could not read prices.json: {e}")

    # 3. watchlist.txt last resort
    if not syms:
        ...
        if wl.exists():
            for line in wl.read_text().splitlines():
                ...
                if line and not line.startswith("#"):
                    add(line)
            print(f"watchlist.txt: {len(syms)} symbols")

    if not syms:
        print("No symbols found in any source.")
        return []

    # Soft check: warn about symbols not in prices.json but do NOT skip them
    # Guidance only needs news -- fundamentals are optional
    try:
        prices = json.loads(Path("prices.json").read_text())
        known  = set(prices.get("quotes", prices).keys())
        missing = [s for s in syms if s not in known]
        if missing:
            print(f"Note: {len(missing)} symbols not yet in prices.json (will still fetch guidance): {chr(44).join(missing[:5])}")
    except:
        pass

    print(f"Fetching guidance for {len(syms)} symbols: {chr(44).join(syms)}")
    return syms
...
{{
  ...
  "summary": "2 sentence max summary of forward outlook"
    ...
    try:
        ...
        print(f"  ! Claude API HTTP {e.code} for {sym}: {err_body[:400]}")
        return None
    except Exception as e:
