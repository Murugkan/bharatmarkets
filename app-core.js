...
function showTab(t, btn){
  S.curTab = t; S.selStock = null;
  const nav = document.getElementById('nav');
  if(nav) Array.from(nav.children).forEach(b => b.classList.remove('active'));
...
function render(){
  ...
  if(!content) return;
  if(S.selStock) { if(typeof renderDrill === 'function') renderDrill(content); return; }
  ...
  switch(S.curTab){
    ...
    case 'upload':    if(typeof renderSettings === 'function') renderSettings(content); break;
  }
}
