...
// Live search across FUND keys + portfolio symbols
function wlSearch(val){
  ...
  // Build search universe from FUND keys + portfolio symbols
  const universe = [...new Set([
    ...Object.keys(FUND),
    ...S.portfolio.map(h=>h.sym)
  ])].map(sym => ({
    sym,
    name:   FUND[sym]?.name   || sym,
    sector: FUND[sym]?.sector || 'Diversified',
  }));

  const exact   = universe.filter(s=>s.sym===q);
  const prefix  = universe.filter(s=>s.sym!==q&&s.sym.startsWith(q));
  const partial = universe.filter(s=>!s.sym.startsWith(q)&&(s.sym.includes(q)||s.name.toUpperCase().includes(q)));
  const hits = [...exact,...prefix,...partial].filter(s=>!already.has(s.sym)).slice(0,8);

  if(!hits.length){
    el.innerHTML=`<div class="srch-results"><div style="padding:12px;text-align:center;font-size:11px;color:var(--mu)">No results — add "${S.wlSearch}" directly or use GFin ↗</div></div>`;
    return;
  }
...
async function headerFundTap(){
  ...
  try{
    ...
    // Clear fundamentals cache so next boot/refresh fetches fresh data
    localStorage.removeItem('fund_cache');
    localStorage.removeItem('fund_cache_ts');
    // Wait ~90s for the Action to complete, then reload
    toast('⏳ Fetching fundamentals — refreshing in 90s…');
    setTimeout(async ()=>{
