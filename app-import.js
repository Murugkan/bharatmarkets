...
function parseCDSLXls(csv){
  ...
  for(let i=headerIdx+1; i<lines.length; i++){
    ...
    if(!sym){
      // ISIN not yet in map — derive best-guess sym, workflow RESOLVE=true will confirm
      sym = name.replace(/[^A-Z0-9]/g,'').slice(0,12).toUpperCase();
      warnings.push({name, sym, reason:'ISIN not in map — symbol will be resolved by workflow'});
    }
...
function parsePortfolioText(text){
  ...
  if(isCDSLExport){
    ...
    for(const line of lines){
      ...
      let sym = ISIN_MAP[isin] || '';
      if(!sym){
        sym = name.replace(/[^A-Z0-9]/g,'').slice(0,12);
      }

      seen.add(isin);
      const info = FUND[sym]||{name,sector};
      results.push({
        sym,
        isin,
    ...
    if(cdslMatch){
      ...
      if(!sym){
        ...
        if(nameMatch){
          ...
          sym = nu.replace(/[^A-Z0-9]/g,'').slice(0,12);
        }
      }
