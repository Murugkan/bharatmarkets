...
# Known variant/legacy sector labels → canonical SECTOR_PROFILES key.
# unified-symbols.json entries are curated by hand, so re-typo'd or
# differently-worded sector names can and will recur (e.g. "IT" vs
# "Information Technology", "Financial Services" vs "Financials"). Without
# this normalization, any mismatch silently falls through to the generic
# "Other" profile — see the 2026-07 audit that found 41/126 symbols doing
# exactly that. Applied case-insensitively before the SECTOR_PROFILES lookup
# so the source file itself doesn't need to stay perfectly spelled forever.
SECTOR_ALIASES = {
...
def resolve_sector(sector: str) -> str:
    """Normalize a raw sector label to a SECTOR_PROFILES key. Exact matches
    pass through untouched; unknown labels are checked against SECTOR_ALIASES
    (case-insensitive); anything still unmatched falls to 'Other'."""
