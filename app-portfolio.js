...
function showPfDebug(){
  ...
  out.push('=== symbols.json sample (first 10 from NSE_DB) ===');
  out.push('NSE_DB length: ' + NSE_DB.length);
  const pfInDb = S.portfolio.filter(h => NSE_DB.find(s=>s.sym===h.sym));
  out.push('Portfolio stocks found in NSE_DB: ' + pfInDb.length + '/' + pfSyms.length);
  out.push('');

  out.push('=== Stocks NOT in NSE_DB ===');
  S.portfolio.forEach(h => {
    if(!NSE_DB.find(s=>s.sym===h.sym)) out.push('  ' + h.sym + ' (isin:' + (h.isin||'—') + ')');
  });
