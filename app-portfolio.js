/**
 * APP-PORTFOLIO.JS - GAP FIX & FULL MAPPING
 * Restoring missing fields: 5D%, GPM%, P/B, and % off Highs.
 */
...
async function renderPortfolio(container) {
    ...
    // COMPREHENSIVE HEADER (Fixed Gaps)
    ...
    window.S.portfolio.forEach((h, index) => {
        ...
        // Color Logic
        const chg1dColor = f.chg1d > 0 ? '#3fb950' : (f.chg1d < 0 ? '#f85149' : '#fff');
        const chg5dColor = f.chg5d > 0 ? '#3fb950' : (f.chg5d < 0 ? '#f85149' : '#fff');
        const roeColor = (parseFloat(f.roe) > 15) ? '#3fb950' : (parseFloat(f.roe) < 0 ? '#f85149' : '#fff');
        const distColor = (val) => (parseFloat(val) > -10) ? '#3fb950' : (parseFloat(val) < -30 ? '#f85149' : '#d29922');
        ...
        html += `<tr style="background:${rowBg}; border-bottom:1px solid #1e3350;">
                    ...
                    <td style="padding:12px; text-align:center; color:${chg1dColor}">${num(f.chg1d)}%</td>
                    <td style="padding:12px; text-align:center; color:${chg5dColor}">${num(f.chg5d)}%</td>
                    ...
                    <td style="padding:12px; text-align:center; color:${distColor(f.w52_pct)}">${num(f.w52_pct)}%</td>
                    <td style="padding:12px; text-align:center; color:${distColor(f.ath_pct)}">${num(f.ath_pct)}%</td>
