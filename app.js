...
// Raw GitHub URL — serves latest committed data files without Pages deploy delay
let RAW_BASE = '';
function getRawBase(){
  const repo = S && S.settings && S.settings.ghRepo ? S.settings.ghRepo.trim() : '';
...
function parseStockXls(csv){
  ...
  for(let i=headerIdx+1; i<lines.length; i++){
    ...
    // Do not guess symbol — workflow resolves from name via Yahoo
    // Use ISIN_MAP only for display; workflow overwrites with confirmed sym
    const sym = ISIN_MAP[isin] || '';
    const info = sym ? (NSE_DB.find(s=>s.sym===sym)||{}) : {};
    imported.push({
      sym, isin,
      cdslName: name,  // preserve original CDSL name for Yahoo resolution
      name:   info.name || name,
...
function parsePortfolioText(text){
  ...
  if(isCDSLExport){
    ...
    for(const line of lines){
      ...
      // No sym guessing — workflow resolves from cdslName via Yahoo
      const sym = ISIN_MAP[isin] || '';
...
async function syncSymbolsDB(){
  ...
  S.watchlist.forEach(w=>{
    ...
    if(!key || seen.has(key)) return;
    seen.add(key);
    // Watchlist sym comes from NSE_DB search — already correct
    entries.push({
      sym:      w.symbol || '',
      name:     w.name   || '',
      isin:     w.isin   || '',
      ...
      source:   ['watchlist'],
      resolved: !!(w.symbol)  // true if user typed/selected a known symbol
    });
  });
...
async function loadFundamentals(forceRefresh){
  try{
    ...
    const r = await fetch(getRawBase()+'/fundamentals.json?t='+Date.now(), {cache:'no-store'});
...
async function refreshPortfolioData(){
  ...
  try{
    ...
    const r = await fetch(getRawBase()+'/prices.json?t='+Date.now(), {cache:'no-store'});
    if(r.ok){
      d = await r.json();
...
async function doClearPortfolio(){
  ...
  if(synced){
    ...
    toast('✅ Portfolio cleared locally');
  }
}
...
function loadAndDrawChart(s){
  ...
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    fetch(getRawBase()+'/charts/'+sym+'.json', {cache:'no-store'})
...
async function loadGuidanceFromGitHub(){
  ...
  try{
    ...
    const r = await fetch(getRawBase()+'/guidance.json?t='+Date.now(), {cache:'no-store'});
    if(r.ok){
      const d = await r.json();
