...
1. News tab working ✓
1. Macro tab populated ✓
1. Portfolio import as textarea ✓
1. Robust CDSL/text parsing ✓
1. Proper candlestick chart ✓
1. Portfolio P&L + sector allocation ✓
1. Watchlist search from full NSE universe ✓
   ═══════════════════════════════════════════════════════════ */
...
function loadState(){
try{S.portfolio=JSON.parse(localStorage.getItem(SK.PORT))||[];}catch(*){}
try{S.watchlist=JSON.parse(localStorage.getItem(SK.WL))||[];}catch(*){}
...
body.innerHTML = `<div style="margin-bottom:10px;font-size:11px;color:var(--tx2);line-height:1.7"> <b style="color:var(--gr2)">Step 1</b> — Long-press the text below → Select All → Copy<br> <b style="color:var(--gr2)">Step 2</b> — In GitHub repo → Add file → Create new file<br> <b style="color:var(--gr2)">Step 3</b> — Name it <code style="color:var(--ac)">portfolio_symbols.txt</code> → Paste → Commit </div> <div style="position:relative"> <textarea id="sym-export-ta" readonly style="width:100%;height:220px;background:#02040a;border:1px solid var(--b2); border-radius:8px;padding:12px;color:var(--gr2);font-family:'JetBrains Mono',monospace; font-size:11px;line-height:1.8;resize:none;box-sizing:border-box;" onclick="this.select()">${txt}</textarea> </div> <button onclick=" const ta=document.getElementById('sym-export-ta'); ta.select(); if(navigator.clipboard){ navigator.clipboard.writeText(ta.value).then(()=>toast('✓ Copied to clipboard!')); } else { document.execCommand('copy'); toast('✓ Copied!'); } " style="width:100%;margin-top:10px;padding:13px;background:var(--gr3);border:1px solid var(--gr); border-radius:10px;color:var(--gr2);font-size:13px;font-weight:700;cursor:pointer; font-family:'Syne',sans-serif;"> 📋 Copy ${syms.length} Symbols to Clipboard </button> <div style="margin-top:10px;font-size:9px;color:var(--mu);text-align:center;font-family:var(--mono)"> Auto-synced on import if GitHub is configured · or commit manually as fallback </div>`;
ov.classList.add(‘on’);
...
`<div class="ti"> <span class="ti-n">${i.name}</span> <div class="ti-r"> <span class="ti-v">${i.val}</span> <span class="ti-c" style="color:${i.chg>=0?'var(--gr2)':'var(--rd2)'}">${i.chg>=0?'▲':'▼'} ${Math.abs(i.chg).toFixed(2)}%</span> </div> </div>`
).join(’’);
}
...
preRows.innerHTML = parsedHoldings.slice(0,10).map(h=>` <div style="display:flex;justify-content:space-between;align-items:center; padding:5px 0;border-bottom:1px solid var(--b1);font-size:10px"> <div> <span style="font-family:var(--mono);font-weight:700;color:var(--tx)">${h.sym}</span> <span style="color:var(--tx3);margin-left:6px;font-size:8px">${h.isin||''}</span> </div> <div style="display:flex;gap:10px;font-family:var(--mono)"> <span style="color:var(--bl2)">×${h.qty}</span> <span style="color:${h.avgBuy>0?'var(--gr2)':'var(--mu)'}"> ${h.avgBuy>0?'₹'+fmt(h.avgBuy):'avg?'} </span> ${h.ltp>0?`<span style="color:var(--tx3)">@₹${h.ltp.toFixed(1)}</span>`:''} </div> </div>`).join(’’) +
...
return `<div style="padding:8px 13px;background:rgba(${st.state==='ok'?'0,208,132':st.state==='error'?'255,59,92':'245,166,35'},.08); border-bottom:1px solid rgba(${st.state==='ok'?'0,208,132':st.state==='error'?'255,59,92':'245,166,35'},.2); font-size:11px;color:${col};font-family:'JetBrains Mono',monospace;display:flex;justify-content:space-between;align-items:center"> <span>${icon} ${st.msg}</span> ${st.state!=='syncing'?`<span style="font-size:9px;color:var(--tx3)">${new Date(st.ts).toLocaleTimeString(‘en-IN’,{hour:‘2-digit’,minute:‘2-digit’,hour12:true})}</span>`:’<span style="font-size:9px;color:var(--tx3)">working…</span>’}
...
return `<span style="display:inline-block;font-size:8px;font-weight:800; padding:2px 7px;border-radius:3px;letter-spacing:.6px; background:${cfg.bg};border:1px solid ${cfg.bd};color:#fff"> ${sig} </span>`;
}
...
c.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:50px 24px;text-align:center"> <div style="font-size:42px;margin-bottom:14px;opacity:.6">💼</div> <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:#fff;margin-bottom:6px">No Portfolio Yet</div> <div style="font-size:12px;color:#8eb0d0;line-height:1.7;margin-bottom:18px">Import your holdings from CDSL statement,<br>CSV, or type manually</div> <button onclick="openImport()" style="background:#f97316;border:none;border-radius:10px;padding:12px 28px;color:#fff;font-size:14px;font-weight:700;font-family:'Syne',sans-serif;cursor:pointer;">📂 Import Holdings</button> </div>`;
return;
}
...
const panel = document.getElementById(‘import-panel’);
const body  = document.getElementById(‘import-panel-body’);
body.innerHTML = `<div style="text-align:center;padding:20px 0 10px"> <div style="font-size:36px;margin-bottom:12px">🗑</div> <div style="font-family:'Syne',sans-serif;font-size:16px;font-weight:700; color:var(--tx);margin-bottom:8px">Clear Portfolio?</div> <div style="font-size:12px;color:var(--tx3);margin-bottom:24px"> This will delete all ${S.portfolio.length} holdings.<br>This cannot be undone. </div> <button onclick=" S.portfolio=[];savePF(); document.getElementById('ov').classList.remove('on'); document.getElementById('import-panel').classList.remove('on'); render(); toast('Portfolio cleared'); " style="width:100%;padding:14px;background:rgba(255,59,92,.15); border:1px solid var(--rd);border-radius:10px;color:var(--rd2); font-size:14px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif; margin-bottom:10px;"> Yes, Clear All </button> <button onclick=" document.getElementById('ov').classList.remove('on'); document.getElementById('import-panel').classList.remove('on'); " style="width:100%;padding:14px;background:var(--s2); border:1px solid var(--b2);border-radius:10px;color:var(--tx3); font-size:14px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif;"> Cancel </button> </div>`;
ov.classList.add(‘on’);
panel.classList.add(‘on’);
}
...
el.innerHTML=`<div class="srch-results"> ${hits.map(h=>`
...
return `<div style="display:flex;align-items:center;gap:8px;padding:4px 0;border-bottom:1px solid var(--b1)"> <span style="font-size:11px;color:var(--tx3);width:80px;flex-shrink:0">${r.step}</span> <span style="font-size:12px">${icon}</span>${fix} </div>`;
}).join(’’);
...
return `<div class="news-item"> <div class="news-src"> <span>${src}</span> <span class="imp-badge imp-${imp}">${imp==='H'?'HIGH':imp==='M'?'MED':'LOW'}</span> <span class="pill pill-bl" style="font-size:7px">${tag}</span> <span>${timeAgo(new Date(item.pubDate))}</span> </div> <div class="news-title">${item.title}</div> ${body?`<div class="news-body">${body}</div>`:''} </div>`;
}).join(’’);
}
...
c.innerHTML=`<div class="drill-wrap"> <div class="drill-hdr"> <button class="back-btn" onclick="closeStock()">← Back</button> <div style="flex:1;min-width:0"> <div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap"> <span class="dsym">${s.symbol}</span> <span class="dchg" style="color:${col}">₹${fmt(s.ltp)} ${bull?'▲':'▼'} ${Math.abs(s.change||0).toFixed(2)}%</span> </div> <div style="font-size:11px;color:var(--tx3);margin-top:2px">${trunc(s.name,40)} · ${s.sector||''}</div> </div> <div class="score-ring" style="border-color:${scoreColor(s.score||65)};background:${scoreColor(s.score||65)}15"> <div class="score-num" style="color:${scoreColor(s.score||65)}">${s.score||65}</div> <div class="score-lbl" style="color:${scoreColor(s.score||65)}">${scoreLabel(s.score||65)}</div> </div> </div> <div class="dtabs"> ${['overview','technical','fundamentals','news','insights'].map(t=>`
...
return `<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.05)"> <span style="font-size:11px;color:var(--tx3);flex-shrink:0;padding-top:1px;min-width:90px">${label}</span> <span style="font-size:11px;font-weight:${empty?'400':'600'};color:${empty?'var(--mu)':(col||'var(--tx1)')};text-align:right;line-height:1.45">${empty?'—':val}</span> </div>`;
}
...
return `<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid rgba(255,255,255,.05)"> <span style="font-size:11px;color:var(--tx3);flex-shrink:0;min-width:90px">${label}</span> <a href="${url}" target="_blank" rel="noopener" style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:4px;background:rgba(100,181,246,.08);border:1px solid rgba(100,181,246,.2);color:#64b5f6;text-decoration:none;white-space:nowrap">🔍 Search</a> </div>`;
}
...
const priceStrip = ` <div style="background:var(--s1);border-bottom:1px solid var(--b1);padding:10px 13px"> <div style="display:flex;justify-content:space-between;align-items:flex-start"> <div> <div style="font-size:24px;font-weight:800;color:var(--tx1);font-family:'JetBrains Mono',monospace;line-height:1">₹${fmt(ltp)}</div> <div style="font-size:11px;font-weight:700;color:${chgCol};margin-top:3px">${(s.change||0)>=0?'▲':'▼'} ${Math.abs(s.change||0).toFixed(2)}%</div> </div> <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 14px;text-align:right"> ${[['Open',fmt(candles[0]?.o||ltp)],['High',fmt(hi)],['Low',fmt(lo)],['Vol',vol+'L']].map(([l,v])=>`
<div><div style="font-size:7px;color:var(--tx3);text-transform:uppercase;letter-spacing:.4px">${l}</div>
<div style="font-size:10px;font-weight:600;color:var(--tx2);font-family:var(--mono)">${v}</div></div>`).join('')} </div> </div> ${s.week52H&&s.week52L&&ltp?`
...
const metricsRow = metrics.length ? ` <div style="overflow-x:auto;display:flex;gap:6px;padding:8px 13px;scrollbar-width:none;border-bottom:1px solid var(--b1)"> ${metrics.map(m=>`
...
const up     = pnl>=0;
return `<div style="margin:8px 12px;padding:10px 13px;background:var(--card);border-radius:10px;border:1px solid var(--b1);border-left:3px solid ${up?'#00d084':'#ff3b5c'}"> <div style="font-size:8px;font-weight:700;color:var(--label);text-transform:uppercase;letter-spacing:.6px;margin-bottom:7px">My Position</div> <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:6px"> ${[['Qty',s.qty],['Avg Buy','₹'+fmt(s.avgBuy)],['P&L',(up?'+':'')+pnlPct+'%'],['Value','₹'+fmt(s.qty*ltp)]].map(([l,v],i)=>`
<div><div style="font-size:7px;color:var(--tx3);margin-bottom:2px">${l}</div>
<div style="font-size:10px;font-weight:700;color:${i===2?(up?'#00e896':'#ff6b85'):'var(--tx1)'};font-family:var(--mono)">${v}</div></div>`).join('')} </div> </div>`;
})() : ‘’;

// ── 4. AI INSIGHTS STRIP ──────────────────────────────────────
const insStrip = ins ? ` <div style="margin:0 12px 0;padding:10px 13px;background:rgba(249,115,22,.07);border:1px solid rgba(249,115,22,.2);border-radius:10px;cursor:pointer" onclick="setDrillTab('insights')"> <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px"> <div style="display:flex;align-items:baseline;gap:6px;flex:1;min-width:0"> <span style="font-size:11px;font-weight:800;color:var(--ac);flex-shrink:0">💡 Portfolio Signal</span> <span style="font-size:11px;color:var(--tx2);line-height:1.45;font-style:italic;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">"${ins.headline||ins.bullets?.[0]||''}"</span> </div> <span style="font-size:11px;font-weight:700;color:var(--ac);flex-shrink:0">→</span> </div> ${ins.action?`<div style="margin-top:6px;font-size:9px;font-weight:700;color:${ins.action.match(/BUY/i)?'#00e896':ins.action.match(/REDUCE|EXIT/i)?'#ff6b85':'#ffbf47'}">${ins.action.split(’—’)[0].trim()}</div>`:''} </div>` : ` <div style="margin:0 12px 0;padding:10px 13px;background:rgba(249,115,22,.03);border:1px dashed rgba(249,115,22,.15);border-radius:10px;cursor:pointer;text-align:center" onclick="setDrillTab('insights')"> <span style="font-size:12px;color:var(--tx3)">💡 No Portfolio Signal — tap to generate</span> </div>`;
...
igBadge = `<div style="margin:8px 12px 0;padding:9px 12px;background:rgba(${ig>=0?'0,232,150':'255,107,133'},.06);border:1px solid rgba(${ig>=0?'0,232,150':'255,107,133'},.2);border-radius:8px;display:flex;justify-content:space-between;align-items:center"> <div><div style="font-size:7px;color:var(--tx3);text-transform:uppercase;letter-spacing:.5px">Implied Earnings Growth</div> <div style="font-size:18px;font-weight:800;color:${ig>=0?'#00e896':'#ff6b85'};font-family:var(--mono);margin-top:2px">${ig>=0?'+':''}${ig}%</div></div> <div style="text-align:right"> <div style="font-size:8px;color:var(--tx3)">Trailing</div><div style="font-size:11px;font-weight:700;color:var(--tx2);font-family:var(--mono)">${f.pe.toFixed(1)}x</div> <div style="font-size:8px;color:var(--tx3);margin-top:3px">Forward</div><div style="font-size:11px;font-weight:700;color:var(--tx2);font-family:var(--mono)">${f.fwd_pe.toFixed(1)}x</div> </div> </div>`;
}

// ── CARD: Action Signal ────────────────────────────────────────
const signalCard = ` <div style="margin:10px 12px 0;border-radius:12px;overflow:hidden;border:1px solid ${actionCol}35"> <div style="padding:12px 14px;background:${actionCol}12"> <!-- Row 1: label + date --> <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"> <span style="font-size:10px;font-weight:700;color:var(--tx3);text-transform:uppercase;letter-spacing:.8px">📋 Concall Signal</span> <span style="font-size:10px;color:var(--tx3)">${updated}</span> </div> <!-- Row 2: action word + conf pill same line, tone separate --> <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap"> <span style="font-size:16px;font-weight:900;color:${actionCol};font-family:'Syne',sans-serif;letter-spacing:.5px">${actionType}</span> ${pill(conf,confCol)} ${pill(tone,toneCol)} </div> <!-- Row 3: reason --> ${actionReason?`<div style="font-size:9px;color:${actionCol};opacity:.85;margin-top:5px;line-height:1.45">${actionReason}</div>`:''} <!-- Row 4: verdict quote — no italics --> ${verdict?`<div style="margin-top:8px;padding:9px 11px;background:rgba(0,0,0,.2);border-radius:8px;font-size:12px;color:var(--tx1);line-height:1.55">”${verdict}”</div>`:''} </div> </div>`;
...
const cid = cardId(id);
return `<div style="margin:6px 12px 0;border-radius:10px;overflow:hidden;border:1px solid var(--b1)"> <div onclick="toggleOvCard('${cid}')" style="display:flex;justify-content:space-between;align-items:center;padding:10px 13px;background:var(--card);cursor:pointer;-webkit-tap-highlight-color:transparent"> <div style="display:flex;align-items:center;gap:7px"> <span style="font-size:13px">${icon}</span> <span style="font-size:12px;font-weight:700;color:var(--tx1)">${title}</span> <div style="width:6px;height:6px;border-radius:50%;background:${col};flex-shrink:0"></div> </div> <span id="${cid}-chev" style="font-size:9px;color:var(--tx3);display:inline-block;transform:${isOpen?'rotate(180deg)':'rotate(0deg)'};transition:transform .2s">▼</span> </div> <div id="${cid}-body" style="background:var(--s2);padding:${isOpen?'10px 13px':'0 13px'};max-height:${isOpen?'800px':'0'};overflow:hidden;transition:max-height .28s ease,padding .2s ease"> ${bodyHtml} </div> </div>`;
}
...
const riskBody  =
`<div style="margin-bottom:10px"> <div style="font-size:10px;font-weight:700;color:#00e896;text-transform:uppercase;letter-spacing:.5px;margin-bottom:5px">Commitments</div> ${commitArr.length ? blist(commitArr,'#00e896') : '<div style="font-size:11px;color:var(--mu)">— Not mentioned in concall</div>'} </div>`
...
return `<div style="padding:10px 13px 14px"> <div class="fund-grid"> ${[ {l:'Market Cap',v:s.mcap||'N/A',sub:'',st:'neutral'}, {l:'P/E Ratio',v:s.pe?s.pe+'x':'—',sub:'Sector ~22x',st:s.pe?(s.pe<18?'good':s.pe<30?'warn':'bad'):'neutral'}, {l:'P/B Ratio',v:s.pb?s.pb+'x':'—',sub:'Price/Book',st:s.pb?(s.pb<2?'good':s.pb<5?'warn':'bad'):'neutral'}, {l:'ROE',v:s.roe?s.roe+'%':'—',sub:'>15% strong',st:s.roe?(s.roe>15?'good':s.roe>10?'warn':'bad'):'neutral'}, {l:'ROCE',v:s.roce?s.roce+'%':'—',sub:'>20% excellent',st:s.roce?(s.roce>20?'good':s.roce>12?'warn':'bad'):'neutral'}, {l:'EPS TTM',v:s.eps?'₹'+fmt(s.eps):'—',sub:'Earnings/Share',st:'neutral'}, {l:'Div Yield',v:s.divYield?s.divYield+'%':'—',sub:'Annual',st:'neutral'}, {l:'Debt/Equity',v:s.debtEq!=null?s.debtEq:'—',sub:'<0.5 safe',st:s.debtEq!=null?(s.debtEq<0.5?'good':s.debtEq<1?'warn':'bad'):'neutral'}, ].map(({l,v,sub,st})=>`
...
return `<div> <div style="padding:10px 12px 4px"> <div class="sec-lbl">News Sources</div> </div> <div id="stock-news-list"> <div style="padding:14px;text-align:center;font-size:11px;color:var(--mu)">Loading news…</div> </div> <div style="padding:10px 12px"> <div class="sec-lbl">Research Links</div> <div style="display:flex;flex-direction:column;gap:6px"> ${[ {l:'NSE Official',u:`https://www.nseindia.com/get-quotes/equity?symbol=${s.symbol}`,c:'#2196f3',d:'Live quotes & filings'}, {l:'Screener.in',u:`https://www.screener.in/company/${s.symbol}/`,c:'#a855f7',d:'Annual reports & ratios'}, {l:'Economic Times',u:`https://economictimes.indiatimes.com/topic/${s.symbol}`,c:'#f97316',d:'Latest news coverage'}, {l:'Moneycontrol',u:`https://www.moneycontrol.com/india/stockpricequote/${s.symbol}/${s.symbol}`,c:'#00d084',d:'Price & analysis'}, {l:'TradingView',u:`https://in.tradingview.com/symbols/NSE-${s.symbol}/`,c:'#64b5f6',d:'Advanced charts'}, {l:'Google News',u:`https://news.google.com/search?q=${encodeURIComponent(s.symbol+’ NSE stock India’)}`,c:'#4285f4',d:'News aggregator'}, ].map(lk=>`
...
return `<div class="news-item" style="margin:0;border-radius:0;border-left:none;border-right:none;border-top:none"> <div class="news-src"> <span>${src}</span> <span class="imp-badge imp-${imp}">${imp==='H'?'HIGH':imp==='M'?'MED':'LOW'}</span> <span class="pill pill-bl" style="font-size:7px">${tag}</span> <span>${timeAgo(new Date(item.pubDate))}</span> </div> <div class="news-title">${item.title}</div> </div>`;
}).join(’’);
}catch(e){
...
const res = document.getElementById(‘filing-result’);
if(!res) return;
res.innerHTML = ` <div style="background:rgba(0,0,0,.2);border-radius:8px;padding:10px;font-size:9px"> <div style="color:var(--tx3);margin-bottom:6px">📄 Latest filing period: <b style="color:var(--tx1)">${filing.quarter}</b></div> <div style="display:flex;flex-direction:column;gap:5px"> <a href="${filing.screenerUrl}" target="_blank" style="display:block;padding:8px 12px;background:rgba(0,208,132,.08);border:1px solid rgba(0,208,132,.3);border-radius:8px;color:#00e896;text-decoration:none;font-weight:700;font-size:10px"> 📊 Screener — ${filing.sym} (Financials + Documents) ↗ </a> <a href="${filing.screenerConc}" target="_blank" style="display:block;padding:8px 12px;background:rgba(0,208,132,.08);border:1px solid rgba(0,208,132,.3);border-radius:8px;color:#00e896;text-decoration:none;font-weight:700;font-size:10px"> 🎙 Screener — ${filing.sym} Concall Transcripts ↗ </a> <div style="display:flex;gap:5px"> <a href="${filing.bseUrl}" target="_blank" style="flex:1;display:block;padding:6px 8px;background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.25);border-radius:6px;color:#818cf8;text-decoration:none;font-size:8px;font-weight:700;text-align:center"> BSE Results ↗ </a> <a href="${filing.nseUrl}" target="_blank" style="flex:1;display:block;padding:6px 8px;background:rgba(99,102,241,.08);border:1px solid rgba(99,102,241,.25);border-radius:6px;color:#818cf8;text-decoration:none;font-size:8px;font-weight:700;text-align:center"> NSE Results ↗ </a> </div> </div> <div style="margin-top:6px;color:var(--tx3);font-size:8px">Tap a link → find the results PDF → save it → attach in Claude.ai</div> </div>`;
}
...
modal.style.cssText = `position:fixed;inset:0;z-index:500; background:rgba(0,0,0,.85); display:flex;flex-direction:column; padding:0;overflow:hidden;`;
modal.innerHTML = `
...
const key = cells[0].toLowerCase().replace(/[^a-z0-9]/g,’*’).replace(/*+/g,’_’);
...
if(m && m[1] && m[2]){
const key = m[1].trim().toLowerCase().replace(/[^a-z0-9]/g,’*’).replace(/*+/g,’_’);
const val = m[2].trim();
...
function get(…keys){
for(const k of keys){
const norm = k.toLowerCase().replace(/[^a-z0-9]/g,’*’).replace(/*+/g,’_’);
// exact match
