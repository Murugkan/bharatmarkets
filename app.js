...
async function syncSymbolsDB(){
  ...
  S.portfolio.forEach(h=>{
    if(!h.sym || seen.has(h.sym)) return;
    seen.add(h.sym);
    entries.push({sym:h.sym, name:h.name||h.sym, sector:h.sector||'Diversified', source:['portfolio']});
  });
  S.watchlist.forEach(w=>{
    if(!w.symbol) return;
    if(seen.has(w.symbol)){
      // already added from portfolio — add watchlist to source
      const e = entries.find(x=>x.sym===w.symbol);
      if(e && !e.source.includes('watchlist')) e.source.push('watchlist');
      return;
    }
    seen.add(w.symbol);
    entries.push({sym:w.symbol, name:w.name||w.symbol, sector:w.sector||'Diversified', source:['watchlist']});
  });
...
function clearPortfolio(){
  ...
  body.innerHTML = `
    <div style="text-align:center;padding:20px 0 10px">
      ...
      <button onclick="
        S.portfolio=[];savePF();
        // Clear stale fundamentals cache so old stocks don't show
        FUND={};fundLoaded=false;
        localStorage.removeItem('fund_cache');
        localStorage.removeItem('fund_cache_ts');
    ...
    <div style="display:flex;gap:6px;align-items:center">
      <div class="search-box" style="margin:0;flex:1">
        ...
        <input class="srch-inp" id="wl-add-inp" type="text"
          ...
          onkeydown="if(event.key==='Enter')wlSearchNSENow()"
          value="${S.wlSearch||''}"/>
      </div>
      <button onclick="wlSearchNSENow()"
...
// Called by NSE↗ button or Enter key — searches Yahoo Finance (CORS-safe)
async function wlSearchNSENow(){
  const val = S.wlSearch.trim();
  if(!val){ toast('Type a symbol or name first'); return; }
  const el = document.getElementById('wl-results');
  if(!el) return;

  const already = new Set(S.watchlist.map(w=>w.symbol));
  el.innerHTML = '<div style="padding:10px;text-align:center;font-size:10px;color:var(--tx3)">Searching NSE…</div>';

  try{
    // Yahoo Finance search — CORS-safe, covers full NSE universe
    const url = 'https://query1.finance.yahoo.com/v1/finance/search?q='
      +encodeURIComponent(val)+'+NSE&quotesCount=8&newsCount=0&enableFuzzyQuery=true';
    ...
    const hits = quotes
      .filter(q=>!already.has(q.symbol.replace('.NS','').replace('.BO','')))
      .map(q=>{
        const sym = q.symbol.replace('.NS','').replace('.BO','');
        return {
          sym,
          name:   q.shortname || q.longname || sym,
          sector: NSE_DB.find(d=>d.sym===sym)?.sector || FUND[sym]?.sector || 'Diversified',
        };
      });

    if(!hits.length){
      el.innerHTML='<div class="srch-results"><div style="padding:12px;text-align:center;font-size:11px;color:var(--mu)">No results from NSE</div></div>';
      return;
    }
    renderWlResults(hits, already, false);
  } catch(e){
    console.warn('Yahoo search failed:', e.message);
    el.innerHTML='<div class="srch-results"><div style="padding:10px;font-size:10px;color:var(--rd2)">Search failed — check connection</div></div>';
  }
}
