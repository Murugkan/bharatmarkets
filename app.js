...
function renderAnalysis(c){
  ...
  c.innerHTML = `<div class="fin" style="padding-bottom:80px">
    ...
    <div style="padding:13px 14px 10px;border-bottom:1px solid var(--b1)">
      ...
      <div style="font-size:9px;color:var(--tx3);margin-top:2px">Concall analysis · ${nDone}/${total} covered</div>
    </div>
    ...
    <div style="padding:8px 13px;border-bottom:1px solid var(--b1)">
      <div style="display:flex;align-items:center;gap:8px;background:var(--s1);border:1px solid var(--b1);border-radius:8px;padding:7px 10px">
        ...
        <input id="an-search" type="text" placeholder="Search symbol or name…"
          ...
          oninput="analysisState.search=this.value;renderAnalysis(document.getElementById('content'))"
          style="flex:1;background:none;border:none;outline:none;color:var(--tx1);font-size:12px;font-family:var(--mono)">
        ${analysisState.search?`<button onclick="analysisState.search='';renderAnalysis(document.getElementById('content'))"
    ...
    <div style="padding:7px 13px 4px">
      <div style="font-size:8px;color:var(--tx3);text-transform:uppercase;letter-spacing:.6px">
        ${sf==='done'?'Covered':'Action Queue'} · ${actionRows.length} stock${actionRows.length!==1?'s':''}
      </div>
    </div>
    <div style="background:var(--card);border-top:1px solid var(--b1);border-bottom:1px solid var(--b1)">
      ${actionRows.map(stockRow).join('')}
    </div>` : (!srch && sf!=='done') ? `
      ...
      <div style="padding:8px 16px 12px;border-bottom:1px solid var(--b1);flex-shrink:0">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            ...
            <div style="font-size:10px;color:var(--tx3);margin-top:2px">${f.name||''} · ${f.sector||''}</div>
          </div>
          <div style="text-align:right">
            <div style="font-family:var(--mono);font-size:13px;color:var(--tx1)">₹${fmt(f.ltp||0)}</div>
            <div style="font-size:9px;color:var(--tx3)">PE: ${f.pe||'—'}x · ₹${posK}</div>
          </div>
        </div>
        <!-- Status + actions row -->
        <div style="display:flex;gap:6px;margin-top:10px">
          ...
          <button onclick="deletePortfolioStock('${sym}')"
            id="btn-del-${sym}"
            style="padding:6px 10px;background:rgba(255,59,92,.08);border:1px solid rgba(255,59,92,.25);
            border-radius:7px;color:#ff6b85;font-size:10px;font-weight:700;cursor:pointer">
            ✕ Remove Stock
      ...
      <div style="flex:1;overflow-y:auto;-webkit-overflow-scrolling:touch">
        ...
        <div style="padding:12px 16px">
          ...
          <div style="font-size:10px;color:var(--tx3);line-height:1.6;margin-bottom:10px;padding:8px 10px;
            ...
            <b style="color:var(--tx2)">Overview tab</b> all come from this paste.
          </div>
          ...
          <div style="display:flex;gap:6px;margin-top:8px">
            ...
            <button onclick="debugAnalysis('${sym}')"
              style="flex:1;background:rgba(100,180,255,.12);border:1px solid rgba(100,180,255,.4);
              border-radius:8px;padding:10px 0;color:#64b5f6;font-size:11px;font-weight:800;cursor:pointer;font-family:'Syne',sans-serif">
              🐛 Debug
            </button>
          </div>
          <div id="debug-panel" style="display:none;margin-top:8px;background:#02040a;border:1px solid #1e3350;
            border-radius:8px;padding:10px;max-height:240px;overflow-y:auto;
            font-family:'JetBrains Mono',monospace;font-size:9px;line-height:1.7"></div>

          ${hasDone?`
          <div style="margin-top:12px;padding:10px;background:rgba(0,0,0,.2);border-radius:8px;border:1px solid var(--b1)">
            <div style="font-size:8px;color:var(--tx3);margin-bottom:5px">
              SAVED · ${new Date(g.updated).toLocaleDateString('en-IN')} · ${g.quarter||''}
            </div>
            <div style="font-size:9px;color:var(--tx2);line-height:1.6">${g.summary||'No summary'}</div>
            <div style="margin-top:6px;display:flex;gap:6px">
              <span style="font-size:8px;font-weight:700;color:${g.tone==='Positive'?'#00e896':g.tone==='Negative'?'#ff6b85':'#ffbf47'}">${g.tone||'Neutral'}</span>
              <span style="font-size:8px;color:var(--tx3)">Confidence: ${g.confidence||'—'}</span>
            </div>
          </div>`:''}
          <!-- bottom safe area -->
...
function deletePortfolioStock(sym){
  ...
  if(btn && btn.dataset.confirm !== '1'){
    ...
    btn.style.borderColor = 'rgba(255,59,92,.5)';
    setTimeout(()=>{
      if(btn){ btn.dataset.confirm=''; btn.textContent='✕ Remove Stock';
        btn.style.background='rgba(255,59,92,.08)';
        btn.style.borderColor='rgba(255,59,92,.25)'; }
    }, 2500);
    return;
