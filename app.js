...
function wlSearch(val){
  ...
  // Show local results immediately while NSE fetch is in progress
  ...
  const localHits = [...exact,...prefix,...partial].filter(s=>!already.has(s.sym)).slice(0,8);
  renderWlResults(localHits, already, true);

  // Debounce NSE live search — fires 400ms after last keystroke
  clearTimeout(_wlSearchTimer);
  _wlSearchTimer = setTimeout(()=>wlSearchNSE(val.trim(), already), 400);
}

async function wlSearchNSE(query, already){
  if(!query) return;
  const el = document.getElementById('wl-results');
  if(!el) return;
  try{
    // NSE autocomplete — full NSE universe, no auth needed
    ...
    const symbols = (data.symbols||[])
      .filter(s=>s.result_sub_type==='equity')
      .slice(0,8);
    if(!symbols.length) return; // keep local results showing

    // Convert NSE format to our format
    const hits = symbols
      .filter(s=>!already.has(s.symbol))
      .map(s=>({
        sym:    s.symbol,
        name:   s.symbol_info || s.symbol,
        sector: NSE_DB.find(d=>d.sym===s.symbol)?.sector || FUND[s.symbol]?.sector || 'Diversified',
      }));
    renderWlResults(hits, already, false);
  } catch(e){
    // CORS or network failure — local results already showing, just leave them
    console.warn('NSE search failed:', e.message);
  }
}
