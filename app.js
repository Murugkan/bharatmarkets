...
<div class="kpi-strip" id="kpi-strip-el">
  ...
  <div class="kpi-status" style="min-width:110px;gap:4px">
    <button onclick="headerPricesTap()" style="display:flex;align-items:center;gap:5px;width:100%;
      ...
      <span style="font-size:8px;font-family:'JetBrains Mono',monospace;flex:1;color:${S.settings._pricesStatus==='ok'?'#00e896':S.settings._pricesStatus==='fail'?'#ff6b85':'#8eb0d0'}">
        ↻ Prices${S.settings._pricesUpdated?' '+new Date(S.settings._pricesUpdated).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:false}):''}</span>
    ...
    <button onclick="headerFundTap()" style="display:flex;align-items:center;gap:5px;width:100%;
      ...
      <span style="font-size:8px;font-family:'JetBrains Mono',monospace;flex:1;color:${S.settings._fundStatus==='ok'?'#00e896':S.settings._fundStatus==='stale'?'#ffbf47':'#8eb0d0'}">
        ↻ Fund${S.settings._fundUpdated?' '+new Date(S.settings._fundUpdated).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:false}):''}</span>
...
async function doClearPortfolio(){
  ...
  await syncSymbolsDB();
  // Trigger fetch to clean prices + fundamentals for removed stocks
...
async function removeFromWL(sym){
  ...
  // Also clear from local FUND/prices if not in portfolio
  const inPF = S.portfolio.find(h=>h.sym===sym);
  if(!inPF){ delete FUND[sym]; }
  saveWL();
  await syncSymbolsDB();
  // Trigger fetch to clean prices + fundamentals for removed stock
  manualTriggerWorkflow('all');
  render();
}
...
async function headerPricesTap(){
  ...
  try{
    // Trigger GitHub Actions workflow for fresh Yahoo fetch (bypasses schedule)
    await manualTriggerWorkflow('prices_only');
    // Also reload prices.json after a short delay to pick up any cached data
    setTimeout(()=>refreshPortfolioData(), 3000);
  } finally {
...
async function manualTriggerWorkflow(fetchType){
  ...
  try{
    ...
    if(r.status === 204){
      const msg = '✅ '+fetchType+' triggered — data updates in ~2 min';
      if(diag){ diag.innerHTML='<span style="color:#00e896">'+msg+'</span>'; }
      S.settings._lastSync = Date.now();
      S.settings._lastSyncOk = true;
      S.settings._lastSyncMsg = fetchType+' triggered '+new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:true});
      saveSettings();
      toast(msg);
      render(); // refresh KPI strip to show updated timestamp
