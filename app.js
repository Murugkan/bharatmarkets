...
<div class="bls-tb" style="gap:6px;flex-wrap:nowrap">
  ...
  <div class="tb-chips">
    ...
    <div class="tb-chip ${S.pfSort==='wt'?'on':''}" onclick="togglePfSort('wt')">Wt% ${S.pfSort==='wt'?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
  </div>
</div>
...
async function refreshPortfolioData(){
  ...
  try{
    ...
    saveSettings();
    if(!S._userSorted) render(); else { buildTicker(); }
    toast(updated>0
...
function boot(){
  loadState();
  loadStaticData().then(()=>{ buildTicker(); if(!S._userSorted) render(); });
  buildTicker();
