/* ═══════════════════════════════════════════════════════════
   BHARATMARKETS PRO v2 — BUILD 20260329-SORT-DEBUG
   SORT FIX: Pos/Neg/Sector chips + debug overlay
   Look for orange DBG button in portfolio toolbar
═══════════════════════════════════════════════════════════ */
...
// ── NSE Universe — full NSE equity list for search + import resolution ──
// Loaded from nse_db.json on boot
let NSE_DB   = [];
let ISIN_MAP = {};
let NSE_NAMES = {};  // UPPER company name → NSE symbol
...
function savePF(){
  ...
  // symbols.json is the single source of truth for data fetching.
...
function exportPortfolioSymbols(){
  ...
  body.innerHTML = `
    <div style="margin-bottom:10px;font-size:11px;color:var(--tx2);line-height:1.7">
      ...
      <b style="color:var(--gr2)">Step 3</b> — GitHub configured? symbols.json syncs automatically on import
    </div>
...
// PORTFOLIO IMPORT — Stock Import XLS, CSV, or manual entry
...
function renderImportPanel(){
  ...
  <!-- Tabs -->
  <div class="imp-tabs">
    <div class="imp-tab on" id="itab-file" onclick="switchImpTab('file')">📁 Stock Import XLS</div>
    <div class="imp-tab" id="itab-paste" onclick="switchImpTab('paste')">📋 Stock Import Text</div>
  ...
  <div class="imp-pane on" id="ipane-file">
    <div class="file-drop" id="file-drop-zone"
      ...
      <div class="file-drop-title">Tap to select Stock Import XLS file</div>
...
// ── Smart symbol resolver — tries multiple name matching strategies ──
function resolveSymFromName(name, isin){
  if(!name) return (isin||'').replace(/[^A-Z0-9]/g,'').slice(0,12);
  const nameUp = name.toUpperCase().replace(/\s+/g,' ').trim();

  // Strategy 1: exact NSE_DB name match
  let found = NSE_DB.find(s=>s.name.toUpperCase()===nameUp);
  if(found) return found.sym;

  // Strategy 2: NSE_DB name starts with first 3 words of CDSL name
  const words = nameUp.split(' ').filter(w=>w.length>1&&!/^(LTD|LIMITED|PVT|PRIVATE|&|AND|OF|THE|INDIA|INDUSTRIES|INDUSTRY|CORP|CO)$/.test(w));
  for(let n=Math.min(3,words.length); n>=1; n--){
    const prefix = words.slice(0,n).join(' ');
    found = NSE_DB.find(s=>s.name.toUpperCase().startsWith(prefix));
    if(found) return found.sym;
  }

  // Strategy 3: CDSL name starts with NSE_DB name (first 2 words)
  found = NSE_DB.find(s=>{
    const nWords = s.name.toUpperCase().split(' ').slice(0,2).join(' ');
    return nameUp.startsWith(nWords) && nWords.length>4;
  });
  if(found) return found.sym;

  // Strategy 4: all meaningful words of CDSL name appear in NSE_DB name
  if(words.length>=2){
    found = NSE_DB.find(s=>{
      const sUp = s.name.toUpperCase();
      return words.slice(0,2).every(w=>sUp.includes(w));
    });
    if(found) return found.sym;
  }

  // Final fallback: strip to alphanumeric (old behaviour)
  return nameUp.replace(/[^A-Z0-9]/g,'').slice(0,12);
}

// ── Stock Import XLS parser — full import with error/warning report ─────
function parseStockXls(csv){
  ...
  for(let i=headerIdx+1; i<lines.length; i++){
    ...
    // Resolve symbol: 1) NSE name map, 2) ISIN map, 3) name search fallback
    // Always derive from company name — never trust CDSL symbol directly
    let sym = NSE_NAMES[name.toUpperCase()]   // exact name match from NSE list
           || ISIN_MAP[isin]                   // ISIN map
           || resolveSymFromName(name, isin);  // fuzzy name search
    if(!NSE_NAMES[name.toUpperCase()] && !ISIN_MAP[isin]){
      warnings.push({name, sym, reason:'Resolved by name search — verify symbol is correct'});
    }
...
function processImportText(text, filename, statusEl){
  // Check if this looks like Stock Import XLS (has the known header)
  const isCDSLXls = /stock name.*isin.*sector.*quantity.*average cost/i.test(text.slice(0,500));

  if(isCDSLXls){
    const result = parseStockXls(text);
    if(result.error){
...
// Multi-format parser: Stock Import XLS export, plain CSV, CDSL PDF text
// Priority: stock export format → Key:Value → numbered list
function parsePortfolioText(text){
  const results = [];
  const seen    = new Set();

  // ── Detect Stock Import XLS/CSV export format ─────────────────────────
  ...
  if(isCDSLExport){
    ...
    for(const line of lines){
      ...
      if(!sym){
        sym = resolveSymFromName(name, isin);
      }
    ...
    console.log('stock export parsed:', results.length, 'holdings');
    return results;
  }
  ...
  for(const line of joined){
    ...
    if(cdslMatch){
      ...
      if(nums.length>=2){
        ...
        // Note: to get avgBuy, use Stock Import XLS/CSV export (Equity_Summary_Details)
...
function applyImport(mode){
  ...
  (async ()=>{
    ...
    // Step 3: Sync symbols.json + trigger fetch workflow
    S._importStatus = { state:'syncing', msg:'Step 3/3 — Syncing & triggering data fetch…', ts: Date.now() };
    updateImportStatus();
    const synced = await syncSymbolsDB();
    if(synced){
      await manualTriggerWorkflow('all');
      S._importStatus = { state:'ok', msg:'✅ Imported & fetch triggered — data loads in ~2 min', ts: Date.now() };
    } else {
      S._importStatus = { state:'ok', msg:'✅ Imported locally — tap ↻ to fetch data', ts: Date.now() };
    }
    updateImportStatus();
  })();
}
...
// ── Unified symbols DB sync — single source of truth ────────────────
// Writes symbols.json to GitHub with all portfolio + watchlist stocks
// Called on: import, watchlist add/remove, portfolio clear
async function syncSymbolsDB(){
  const token = S.settings.ghToken?.trim();
  const repo  = S.settings.ghRepo?.trim();
  if(!token || !repo) return;

  // Build unified symbol list: portfolio + watchlist, deduped
  const seen = new Set();
  const entries = [];

  S.portfolio.forEach(h=>{
    const key = h.isin || h.sym;
    if(!key || seen.has(key)) return;
    seen.add(key);
    entries.push({
      sym:    h.sym   || '',          // tentative — workflow confirms via Yahoo
      name:   h.cdslName || h.name || '',  // full company name for Yahoo resolution
      isin:   h.isin  || '',
  ...
  S.watchlist.forEach(w=>{
    ...
    entries.push({
      ...
      name:   w.name   || '',
      isin:   w.isin   || '',
      sector: w.sector || 'Diversified',
      source: ['watchlist']
    });
  });

  const encoded = btoa(unescape(encodeURIComponent(JSON.stringify(entries, null, 0))));
  const headers = {'Authorization':'token '+token,'Content-Type':'application/json','Accept':'application/vnd.github.v3+json'};
  const fileUrl = 'https://api.github.com/repos/'+repo+'/contents/symbols.json';
  ...
  try{
    ...
    const body = {message:'symbols: update '+new Date().toISOString().slice(0,10), content: encoded};
    if(sha) body.sha = sha;
    const put = await fetch(fileUrl, {method:'PUT', headers, body:JSON.stringify(body)});

    if(put.ok){
      console.log('symbols.json synced:', entries.length, 'stocks');
      S.settings._lastSync    = Date.now();
      S.settings._lastSyncOk  = true;
      S.settings._lastSyncMsg = entries.length+' symbols synced';
      saveSettings();
      return true;
    } else {
      const err = await put.json().catch(()=>({}));
      console.warn('symbols.json sync failed:', err.message||put.status);
      return false;
    }
  } catch(e){
    console.warn('syncSymbolsDB error:', e.message);
    return false;
  }
}
...
// Data: portfolio holdings (stock import) + fundamentals.json
...
function sortRows(rows, skey, sdir) {
  rows.sort((a,b) => {
    ...
    switch(skey) {
      ...
      case 'sector': av=(a.sector&&a.sector!=='—'?a.sector:'zzz'); bv=(b.sector&&b.sector!=='—'?b.sector:'zzz'); break;
      ...
      case 'wt':
      default:       av=a.ltp>0?a.qty*a.ltp:0; bv=b.ltp>0?b.qty*b.ltp:0; break;
    }
...
function renderPortfolio(c){
  ...
  if(!S.portfolio.length){
    c.innerHTML = `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:50px 24px;text-align:center">
      ...
      <div style="font-size:12px;color:#8eb0d0;line-height:1.7;margin-bottom:18px">Import your holdings from stock statement,<br>CSV, or type manually</div>
      <button onclick="openImport()" style="background:#f97316;border:none;border-radius:10px;padding:12px 28px;color:#fff;font-size:14px;font-weight:700;font-family:'Syne',sans-serif;cursor:pointer;">📂 Import Stocks</button>
    </div>`;
    return;
  }
...
.pn-p{background:#7a6010;color:#fff;padding:1px 4px;border-radius:2px;font-size:8px;font-weight:700;}
.pn-n{background:#7a6010;color:#fff;padding:1px 4px;border-radius:2px;font-size:8px;font-weight:700;}
</style>
...
<div class="kpi-strip" id="kpi-strip-el">
  ...
  <div class="kpi-status" style="min-width:115px;gap:4px">
    <button onclick="headerPricesTap()" style="display:flex;align-items:center;gap:5px;width:100%;
      padding:4px 6px;border-radius:5px;cursor:pointer;border:1px solid rgba(0,208,132,.2);
      background:rgba(0,208,132,.06);text-align:left;transition:background .15s"
      ontouchstart="this.style.background='rgba(0,208,132,.15)'" ontouchend="this.style.background='rgba(0,208,132,.06)'">
      <span style="width:6px;height:6px;border-radius:50%;flex-shrink:0;background:${S.settings._pricesStatus==='ok'?'#00d084':S.settings._pricesStatus==='fail'?'#ff3b5c':'#4a6888'}"></span>
      <span id="kpi-prices-lbl" style="font-size:8px;font-family:'JetBrains Mono',monospace;flex:1;color:${S.settings._pricesStatus==='ok'?'#00e896':S.settings._pricesStatus==='fail'?'#ff6b85':'#8eb0d0'}">
        ${S.settings._pricesStatus==='ok'?'✓':'↻'} Prices${S.settings._pricesUpdated?' '+new Date(S.settings._pricesUpdated).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:false}):''}</span>
      <span class="kpi-sico" id="hdr-prices-spin" style="font-size:9px"></span>
    </button>
    <button onclick="headerFundTap()" style="display:flex;align-items:center;gap:5px;width:100%;
      padding:4px 6px;border-radius:5px;cursor:pointer;border:1px solid rgba(156,39,176,.2);
      background:rgba(156,39,176,.06);text-align:left;transition:background .15s"
      ontouchstart="this.style.background='rgba(156,39,176,.15)'" ontouchend="this.style.background='rgba(156,39,176,.06)'">
      <span style="width:6px;height:6px;border-radius:50%;flex-shrink:0;background:${S.settings._fundStatus==='ok'?'#00d084':S.settings._fundStatus==='stale'?'#ffbf47':'#4a6888'}"></span>
      <span id="kpi-fund-lbl" style="font-size:8px;font-family:'JetBrains Mono',monospace;flex:1;color:${S.settings._fundStatus==='ok'?'#00e896':S.settings._fundStatus==='stale'?'#ffbf47':'#8eb0d0'}">
        ${S.settings._fundStatus==='ok'?'✓':'↻'} Fund${S.settings._fundUpdated?' '+new Date(S.settings._fundUpdated).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit',hour12:false}):''}</span>
      <span class="kpi-sico" id="hdr-fund-spin" style="font-size:9px"></span>
    </button>
  </div>
</div>
...
<div class="bls-tb" style="gap:6px;flex-wrap:nowrap">
  ...
  <div class="tb-chips">
    ...
    <div class="tb-chip ${S.pfSort==='sector'?'on':''}" onclick="togglePfSort('sector')">Sec ${S.pfSort==='sector'?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
    <div class="tb-chip ${S.pfSort==='pnlpct'?'on':''}" onclick="togglePfSort('pnlpct')">P&L% ${S.pfSort==='pnlpct'?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
    <div class="tb-chip ${S.pfSort==='wt'||S.pfSort===''?'on':''}" onclick="togglePfSort('wt')">Wt% ${(S.pfSort==='wt'||S.pfSort==='')?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
  </div>
</div>
...
<div style="margin:10px 12px;padding:12px;background:rgba(245,166,35,.06);border:1px solid rgba(245,166,35,.25);border-radius:8px;font-size:10px;color:#8eb0d0;font-family:'JetBrains Mono',monospace;line-height:2">
  ...
  <button onclick="exportPortfolioSymbols()" style="margin-top:8px;width:100%;padding:7px;background:rgba(100,181,246,.06);border:1px solid rgba(100,181,246,.2);border-radius:6px;color:#64b5f6;font-size:10px;font-weight:600;cursor:pointer;font-family:'JetBrains Mono',monospace;">
    📤 Manual fallback — copy symbols to clipboard
  </button>
</div>`:''}
...
function togglePfSort(k){
  ...
  render();
  // Scroll table to top so sorted order is visible
  requestAnimationFrame(()=>{
    const tbl = document.querySelector('.bls-table-outer');
...
async function refreshPortfolioData(){
  ...
  try{
    ...
    toast(updated>0
      ...
      : '⚠ No prices — trigger Fetch Prices from settings');
  }
}
...
async function doClearPortfolio(){
  ...
  localStorage.removeItem('fund_cache');
  localStorage.removeItem('fund_cache_ts');
  localStorage.removeItem('bm_guidance');
  _deleteQueue.clear(); // wipe any stale queue

  document.getElementById('ov').classList.remove('on');
  document.getElementById('import-panel').classList.remove('on');
  render();

  // 2. Sync symbols.json — will be empty [] since portfolio+watchlist empty
  const synced = await syncSymbolsDB();
  if(synced){
    toast('✅ Portfolio cleared — triggering data cleanup…');
    // 3. Trigger fetch to wipe prices, fundamentals, guidance, charts
    await manualTriggerWorkflow('all');
  } else {
    toast('✅ Portfolio cleared locally — configure GitHub to sync');
  }
}
...
function clearPortfolio(){
  ...
  body.innerHTML = `
    <div style="text-align:center;padding:20px 0 10px">
      ...
      <button onclick="doClearPortfolio()" style="width:100%;padding:14px;background:rgba(255,59,92,.15);
...
function renderWatchlist(c){
  ...
  <div style="padding:10px 13px;background:var(--bg);border-top:1px solid var(--b1);position:sticky;bottom:56px">
    <div style="display:flex;gap:6px;align-items:center">
      ...
      <button onclick="window.open('https://www.google.com/finance/quote/'+encodeURIComponent(S.wlSearch.trim())+':NSE')"
        style="flex-shrink:0;padding:6px 10px;background:rgba(249,115,22,.15);
        border:1px solid rgba(249,115,22,.4);border-radius:7px;color:var(--ac);
        font-size:11px;font-weight:700;cursor:pointer;font-family:'Syne',sans-serif"
        title="Search on Google Finance">
        GFin↗
      </button>
    </div>
    <div id="wl-results" style="margin-top:6px"></div>
  </div>
...
function wlSearch(val){
  ...
  const partial = NSE_DB.filter(s=>!s.sym.startsWith(q)&&(s.sym.includes(q)||s.name.toUpperCase().includes(q)));
  const hits = [...exact,...prefix,...partial].filter(s=>!already.has(s.sym)).slice(0,8);
  renderWlResults(hits, already, true);
}



function renderWlResults(hits, already, isLocal){
  const el = document.getElementById('wl-results');
  if(!el) return;
  if(!hits.length){
    if(!isLocal) el.innerHTML=`<div class="srch-results"><div style="padding:12px;text-align:center;font-size:11px;color:var(--mu)">No results</div></div>`;
    return;
  }
...
async function addToWL(sym){
  ...
  saveWL();
  await syncSymbolsDB();
  // Trigger fetch to resolve symbol + get prices/fundamentals immediately
  manualTriggerWorkflow('all');
  render();
}

// Remove stock from watchlist + sync to GitHub
async function removeFromWL(sym){
  S.watchlist = S.watchlist.filter(w=>w.symbol!==sym);
  // Clear local data immediately if not in portfolio
  const inPF = S.portfolio.find(h=>h.sym===sym);
  if(!inPF){
    delete FUND[sym];
    delete GUIDANCE[sym];
    delete chartCache[sym];
  }
  saveWL();
  const synced = await syncSymbolsDB();
  if(synced) await manualTriggerWorkflow('all');
  render();
}

// ── Symbol footprint deletion — complete wipe of all traces ─────────
// Handles both individual (watchlist) and mass (portfolio clear)
// Clears: memory, localStorage, GitHub files (prices, fundamentals, guidance, charts, symbols)

const _deleteQueue = new Set(); // accumulate symbols before GitHub sync

function queueDeleteSymbol(sym){
  // Only delete if not in the OTHER list
  const inPF = S.portfolio.find(h=>h.sym===sym);
  const inWL = S.watchlist.find(w=>w.symbol===sym);
  if(inPF || inWL) return; // still referenced — keep
  ...
  // 2. Queue for GitHub cleanup
  _deleteQueue.add(sym);
}

async function flushDeleteQueue(){
  if(!_deleteQueue.size) return;
  const syms = [..._deleteQueue];
  _deleteQueue.clear();

  const token = S.settings.ghToken?.trim();
  const repo  = S.settings.ghRepo?.trim();

  // 3. Clear localStorage caches (rebuild from remaining data)
  try{
    const cached = localStorage.getItem('fund_cache');
    if(cached){
      const d = JSON.parse(cached);
      syms.forEach(s=>{ delete d.stocks?.[s]; });
      localStorage.setItem('fund_cache', JSON.stringify(d));
    }
  }catch(_){}

  try{
    const g = localStorage.getItem('bm_guidance');
    if(g){
      const d = JSON.parse(g);
      syms.forEach(s=>{ delete d[s]; });
      localStorage.setItem('bm_guidance', JSON.stringify(d));
    }
  }catch(_){}

  if(!token || !repo){
    console.log('deleteQueue: no GitHub config, local cleanup only');
    return;
  }

  // 4. Trigger fetch workflow — it will clean prices.json, fundamentals.json,
  //    guidance.json and charts/SYM.json for all removed symbols
  //    (fetch scripts compare active symbols.json against existing data)
  await manualTriggerWorkflow('all');
  console.log('deleteQueue: cleanup triggered for', syms.join(', '));
}
...
async function headerPricesTap(){
  const spin = document.getElementById('hdr-prices-spin');
  const lbl  = document.getElementById('kpi-prices-lbl');
  if(spin){ spin.textContent='⏳'; spin.style.color='#ffbf47'; }
  if(lbl) { lbl.textContent='⏳ Prices…'; lbl.style.color='#ffbf47'; }
  try{
    await manualTriggerWorkflow('prices_only');
...
async function headerFundTap(){
  ...
  const lbl  = document.getElementById('kpi-fund-lbl');
  if(spin){ spin.textContent='⏳'; spin.style.color='#ffbf47'; }
  if(lbl) { lbl.textContent='⏳ Fund…'; lbl.style.color='#ffbf47'; }
  try{
...
async function manualTriggerWorkflow(fetchType){
  ...
  try{
    ...
    if(r.status === 204){
      ...
      const msg = '✅ '+fetchType+' triggered '+t+' — updates in ~2 min';
      S.settings._lastSync = Date.now();
      S.settings._lastSyncOk = true;
      S.settings._lastSyncMsg = msg;
      saveSettings();
      toast(msg);
      // Update KPI label directly without full render
      const isPrices = fetchType.includes('price') || fetchType==='all';
      const isFund   = fetchType.includes('fund')  || fetchType==='all';
      if(isPrices){
        const lbl = document.getElementById('kpi-prices-lbl');
        if(lbl){ lbl.textContent='⏳ Prices triggered '+t; lbl.style.color='#ffbf47'; }
      }
      if(isFund){
        const lbl = document.getElementById('kpi-fund-lbl');
        if(lbl){ lbl.textContent='⏳ Fund triggered '+t; lbl.style.color='#ffbf47'; }
      }
...
function drawCandlestick(s){
  ...
  if(!candles.length){
    ...
    if(s._noChartData){
      ...
      ctx.fillText('Add to watchlist → trigger Fetch Prices in settings', W2/2, H2/2+4);
  ...
  c.innerHTML = `<div style="padding:10px 13px 80px;display:flex;flex-direction:column;gap:12px">
    ...
    <div style="background:var(--card);border:1px solid var(--b1);border-radius:12px;overflow:hidden">
      ...
      <div style="padding:10px 13px;font-size:10px;color:var(--tx3);line-height:1.8;border-bottom:1px solid var(--b1)">
        <b style="color:var(--gr2)">Recommended — Stock Import XLS</b><br>
        CDSL Easiest → Portfolio → Equity Summary Details → Download XLS<br>
        Single file: symbol, sector, qty, avg buy — complete import<br><br>
        <b style="color:var(--yw2)">Fallback — Stock Import Text or Manual CSV</b><br>
        Copy-paste from stock statement · or type <code style="color:#64b5f6">SYMBOL, QTY, AVG</code> manually<br>
...
async function loadStaticData() {
  try {
    ...
    if (r.ok) {
      ...
      ISIN_MAP  = d.isin   || {};
      NSE_NAMES = d.names  || {};
      console.log('nse_db.json loaded:', NSE_DB.length, 'stocks,', Object.keys(ISIN_MAP).length, 'ISINs,', Object.keys(NSE_NAMES).length, 'names');
    }
