...
function renderPortfolio(c){
  ...
  if(S.pfSort==='sector') dbgLog('RENDER TOP5: '+rows.slice(0,5).map(h=>h.sym+':'+h.sector).join(' | '));
...
<div class="bls-tb" style="gap:6px;flex-wrap:nowrap">
  ...
  <button onclick="toggleDbg()" style="flex-shrink:0;padding:4px 8px;background:rgba(249,115,22,.15);border:1px solid rgba(249,115,22,.4);border-radius:4px;color:#f97316;font-size:9px;font-weight:700;cursor:pointer;font-family:'JetBrains Mono',monospace">DBG</button>
  <div class="tb-chips">
    ...
    <div class="tb-chip ${S.pfSort==='pos'?'on':''}" onclick="togglePfSort('pos')">Pos ${S.pfSort==='pos'?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
    <div class="tb-chip ${S.pfSort==='neg'?'on':''}" onclick="togglePfSort('neg')">Neg ${S.pfSort==='neg'?(S.pfSortDir==='desc'?'↓':'↑'):''}</div>
...
// ── Sort / filter controls ──────────────────────────────────
function setPfFilter(f){S.pfFilter=f;dbgLog('setPfFilter: '+f+' | pfSort='+S.pfSort);if(S.curTab==='portfolio')render();}
...
function togglePfSort(k){
  dbgLog('CLICK togglePfSort("'+k+'") | before: pfSort='+S.pfSort+' dir='+S.pfSortDir);
  if(S.pfSort===k) S.pfSortDir=S.pfSortDir==='desc'?'asc':'desc';
  else{ S.pfSort=k; S.pfSortDir='desc'; }
  dbgLog('AFTER set: pfSort='+S.pfSort+' dir='+S.pfSortDir);
  if(k==='sector'){
    try{
      const pf=S.portfolio.map(mergeHolding);
      dbgLog('PORTFOLIO LEN: '+pf.length);
      const uniq=[...new Set(pf.map(h=>h.sector||'EMPTY'))];
      dbgLog('UNIQUE SECTORS('+uniq.length+'): '+uniq.slice(0,8).join(' | '));
      // Show first 5 rows after sort to confirm order
      const rows=[...pf];
      rows.sort((a,b)=>{
        const av=normSector(a.sector||'');
        const bv=normSector(b.sector||'');
        return S.pfSortDir==='asc'?av.localeCompare(bv):bv.localeCompare(av);
      });
      dbgLog('SORTED TOP5: '+rows.slice(0,5).map(h=>h.sym+':'+h.sector).join(' | '));
    }catch(e){ dbgLog('SECTOR DBG ERR: '+e.message); }
  }
  render();
  dbgLog('AFTER render: pfSort='+S.pfSort+' dir='+S.pfSortDir);
...
function dbgLog(msg){
  ...
  _dbgLogs.unshift('['+t+'] '+msg);
  if(_dbgLogs.length>40) _dbgLogs.pop();
  renderDbg();
}
...
function renderDbg(){
  ...
  el.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:6px">'
    +'<b style="color:#f97316">DEBUG</b>'
    +'<span style="color:#64b5f6">pfSort=<b>'+S.pfSort+'</b> dir=<b>'+S.pfSortDir+'</b></span>'
    +'<button onclick="document.getElementById(&quot;dbg-overlay&quot;).style.display=&quot;none&quot;" style="background:none;border:1px solid #333;color:#fff;padding:1px 6px;cursor:pointer;border-radius:3px">✕</button>'
    +'</div>'
    + _dbgLogs.map(l=>'<div style="border-bottom:1px solid #111;padding:2px 0;word-break:break-all">'+l+'</div>').join('');
}
...
function toggleDbg(){
  ...
  if(!el){
    ...
    el.id = 'dbg-overlay';
    el.style.cssText = 'position:fixed;bottom:70px;left:8px;right:8px;max-height:55vh;overflow-y:auto;'
      +'background:rgba(2,4,10,0.97);border:1px solid #f97316;border-radius:10px;'
      +'padding:10px 12px;font-family:JetBrains Mono,monospace;font-size:9px;'
      +'color:#c8dff5;z-index:9999;line-height:1.6;';
...
async function refreshPortfolioData(){
  ...
  try{
    ...
    saveSettings();
    dbgLog('ASYNC render: refreshPortfolio | pfSort='+S.pfSort);
    render();
    toast(updated>0
...
function boot(){
  ...
  loadFundamentals()
    ...
    .then(() => { dbgLog('ASYNC render: loadFundamentals.then | pfSort='+S.pfSort); buildTicker(); render(); });

  loadGuidanceFromGitHub()
    .catch(e => console.warn('loadGuidanceFromGitHub failed:', e))
    .then(() => { dbgLog('ASYNC render: loadGuidance.then | pfSort='+S.pfSort); render(); });
}
boot();